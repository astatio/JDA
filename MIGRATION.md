...
Verification: `./gradlew clean check` green — 506 tests / 0 failures, `apiCheck` baseline unchanged, `verifyBytecodeVersion`, `spotlessCheck`/`rewriteDryRun`, and `detekt`. The eight files still Java under `internal.components` are the larger composite components (`actionrow`, `buttons`, `checkboxgroup`, `container`, `label`, `radiogroup`, `section`, `textinput`) and are the next batch.
